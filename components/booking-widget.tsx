...
export function BookingWidget({ compact = false }: { compact?: boolean }) {
  ...
  return <>
    <div className={`booking-widget glass-panel ${compact ? "booking-compact" : ""}`} id="book">
      <div className="booking-intro"><span className="star-glyph">✦</span><span><small>WEBSITE UNDER MAINTENANCE</small><b>Direct Phone &amp; Event Inquiries</b></span></div>
      ...
      <button className="gold-button availability" onClick={() => setResults(true)}>Direct Inquiry <ArrowRight size={18}/></button>
    </div>
    {results && <div className="results-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setResults(false)}>
      <section className="results-panel" aria-modal="true" role="dialog" aria-label="Available rooms">
        ...
        <div className="results-head"><p className="kicker">✦ WEBSITE UNDER MAINTENANCE ✦</p><h2>Direct Booking <em>Inquiries.</em></h2><p>{format(new Date(checkIn), "dd MMM")} — {format(new Date(checkOut), "dd MMM yyyy")} · {guests} guests</p></div>
        <p className="preview-note">✦ WEBSITE UNDER MAINTENANCE ✦ Our online booking system is currently undergoing maintenance. Please call us at +91 94250 88369 or +91 98934 88369 to confirm availability and reserve directly.</p>
