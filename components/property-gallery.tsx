...
export function PropertyGallery({ eyebrow, title, copy, images }: GallerySet) {
  ...
  return <section className="property-gallery-section">
    ...
    <div className="property-gallery-grid">{images.map((image, index) => <button key={image} className={`gallery-tile tile-${index % 7}`} onClick={() => setSelected(index)} aria-label={`Open photograph ${index + 1} of ${images.length}`}><img src={image} alt={`${title} · photograph ${index + 1}`} loading={index < 3 ? "eager" : "lazy"}/><span>{String(index + 1).padStart(2, "0")} / {String(images.length).padStart(2, "0")}<Expand/></span></button>)}</div>
